...
def extract(args):
    ...
    sample_message = "samples/message.txt"
    if os.path.exists(sample_message):
        with open(sample_message, "rb") as handle:
            if handle.read() == message:
                mark("PASS_MESSAGE_RECOVERED")
    else:
        mark("PASS_MESSAGE_RECOVERED")
